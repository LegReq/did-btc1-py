...
async def generate_external_test_vector():
    ...
    folder_path = f"./TestVectors/{did_path}"
    ...
    resolved_doc = await resolver.resolve(identifier, resolution_options)
        
    print(json.dumps(resolved_doc.serialize(), indent=2))
    ...
    address = new_beacon_sk.point.p2wpkh_address(network=network)
    ...
    updated_doc = await did_manager.finalize_update_payload(updater, cap_vm_id, cap_sk, beacon_id, beacon_sk)
